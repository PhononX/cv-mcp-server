...
export const TOOL_DOCS: ToolDocRegistry = {
  ...
  list_ai_actions: {
    ...
    whenToUse:
      'Before calling `run_ai_action` or `summarize_conversation`, to find a `prompt_id`. Also to show the user which AI Actions exist. Filter by `owner_type` (`user` = your own, `workspace` = shared, `system` = Carbon Voice built-ins).',
    whenNotToUse:
    ...
    responseShape:
      'Array of `{id, name, description?, prompt, owner_type, workspace_id?, response_format?, created_at, last_updated_at}`. Use `id` as `prompt_id` elsewhere.',
  ...
  run_ai_action: {
    ...
    whenNotToUse:
      '`summarize_conversation` if you want a whole conversation summarized and would otherwise have to list its messages yourself — it does that selection for you.',
    prerequisites: [
      { field: 'prompt_id', fromTool: 'list_ai_actions', fromField: 'id' },
      { field: 'message_ids', fromTool: 'list_messages', fromField: 'results[].id' },
    ],
    example: {
    ...
    responseShape:
      '`{id, prompt_id, message_ids, creator_id, channel_id?, workspace_id?, responses: [{language, text?, markdown?, html?, json?}], created_at}`. The generated output is in `responses[]`, one entry per language.',
    commonErrors: [
      {
        code: 'BAD_REQUEST',
  ...
  summarize_conversation: {
    ...
    whenToUse:
      'You want a conversation summarized and have a `conversation_id`. Message selection is handled for you — omit `message_ids` and the most recent messages are used.',
    whenNotToUse:
  ...
  get_ai_action_responses: {
    ...
    whenToUse:
      'You want existing output rather than a fresh run — e.g. showing what an AI Action already produced for a message or conversation. Combine `prompt_id`, `message_id` and `channel_id` to narrow.',
    whenNotToUse:
      '`run_ai_action` (or `summarize_conversation`) to generate new output; this tool only reads what already exists and returns an empty array if nothing has been generated.',
    example: { channel_id: 'conv-abc', limit: 10 },
    responseShape:
      'Array of `{id, prompt_id, creator_id, message_ids, channel_id?, workspace_id?, responses: [{language, text?, markdown?, html?, json?}], created_at}`.',
