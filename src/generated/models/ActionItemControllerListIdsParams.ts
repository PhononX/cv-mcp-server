 ...
 */
import type { ActionItemControllerListIdsDirection } from './ActionItemControllerListIdsDirection';
import type { ActionItemStatus } from './ActionItemStatus';

export type ActionItemControllerListIdsParams = {
  direction?: ActionItemControllerListIdsDirection;
  /**
   * Number of items to return (min: 1, max: 1000)
   */
  limit?: number;
  /**
   * Anchor timestamp to start from. Uses updated_at. Ignored when starting_after/ending_before is provided.
   */
  date?: string;
  /**
   * Cursor to fetch the next page (use value from next_cursor).
   */
  starting_after?: string;
  /**
   * Cursor to fetch the previous page (use value from previous_cursor).
   */
  ending_before?: string;
  /**
   * Filter by assigned_to (user_id). Use 'null' to filter by unassigned items.
   */
  assigned_to?: string;
